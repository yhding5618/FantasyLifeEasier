...
此处为自动识别并完成任意职业的制作小游戏功能，目前已经实现自动检测特殊操作进度和起始工作台位置，无需手动调整任何参数。
...
1. 开始任意制作小游戏。
1. 在脚本内点击“连续操作”按钮，脚本会自动识别并执行所有操作，直到进入“制作完成”页面。

“循环再次制作/继续重制”：

这个功能会在完成一次“连续操作”之后自动选择“再次制作”/“继续重制”，再进行下一次“连续操作”，循环次数可以自定义，0表示无限循环。这里有三种用法供参考：

1. “再次制作”：素材不多的时候进行多次数量为一的制作，这样每次超大成功的制作都可以获得四份产物，最大化利用素材。
1. “再次制作”：挂机刷随性制作，可以配合“自动截图”，脚本会在每次进入“制作完成”页面时（按F12）截图，方便查看记录。
1. “继续重制”：重制武器属性，脚本会在进入4选2的“技能重置”页面后进入等待状态（最多180秒，超时会自动中止），这时候需要手动选择技能，然后按Tab再按空格确认，之后脚本会自动继续下一次重制。
...
1. 在脚本内点击“自动循环熟成车”按钮，脚本会自动循环以下步骤：
    ...
    1. 落地后往前直走到传送阵熟成房的传送阵。
    1. 这个时候可以通知其他成员加入房间。
    1. 如果选择了“自动发送QQ”，脚本会切换到QQ窗口并发送“千熟全自动发车，词...，密...”。
    1. 脚本会在识别到所有已加入房间的成员都出现在小地图上的传送阵附近之后开始传送。
    1. 脚本会自动传送到Boss房，然后等待Boss被成员击杀并回到据点。
    1. 脚本会自动解散房间并返回到联机柜台。

> 目前尚未实现全自动寻路，不要让成员在脚本自动暂停前就加入房间，否则会打断脚本的自动前进，可能导致无法走到正确的目的地。
>
> 如果需要自动发送QQ消息，需要提前登录好QQ并打开想要发送消息的聊天窗口。
...
1. 在游戏内站到玛可面前。
1. 在脚本内点击“保存云”按钮，或者手动保存云存档。
1. 在脚本内选择盲盒类型（女神草或女神花），然后选择你要抽取的盲盒在列表内的位置（从1开始计数，例如：梦幻盒为3）最后选择你要抽取的数量（可以先选择1个进行测试）。
